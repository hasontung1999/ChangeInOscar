...
import sys
sys.path.append('/content/drive/MyDrive/FPT/Image_Captioning/Oscar/')

from oscar.utils import my_logger
...
class CaptionTSVDataset(Dataset):
    ...
    def get_image_features(self, img_idx):
        ...
        return torch.Tensor(np.array(features)) #UserWarning: The given NumPy... -> Use copy of it np.array()
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    global_loss, global_acc = args.current_loss*args.global_step, args.current_acc*args.global_step
    ...
    for epoch in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        # add_val_log = {}
        add_val_metrics_log = {}
        add_train_log['epoch'] = epoch
        # add_val_log['epoch'] = epoch
        add_val_metrics_log['epoch'] = epoch
        for step, (img_keys, batch) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    progbar.update(global_step, [('lr', optimizer.param_groups[0]["lr"]),
                    ('batch_loss', loss.item()), ('curr_loss', global_loss/global_step), 
                    ('batch_score', batch_acc.item()), ('curr_score', global_acc/global_step)])
                    # ('score', '{:.4f} ({:.4f})'.format(batch_acc.item(), global_acc/global_step))])
        ...
        add_train_log['loss'] = global_loss/global_step
        add_train_log['acc'] = global_acc/global_step
        log_train.add_val(**add_train_log)
        ...
        if args.evaluate_during_training: 
            ...
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            #|          Get (epoch, loss, score) for valid dataset          |
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            # val_global_step, val_global_loss, val_global_acc = 0, 0.0, 0.0
            # for val_step, (val_img_keys, val_batch) in enumerate(val_dataset):
            #   val_batch = tuple(t.to(args.device) for t in val_batch)
            #   if not args.scst:

            #       val_inputs = {'input_ids': val_batch[0], 'attention_mask': val_batch[1],
            #           'token_type_ids': val_batch[2], 'img_feats': val_batch[3], 
            #           'masked_pos': val_batch[4], 'masked_ids': val_batch[5]
            #       }
            #       val_outputs = model(**val_inputs)
            #       val_loss, val_logits = val_outputs[:2]
            #       val_masked_ids = val_inputs['masked_ids']
            #       val_masked_ids = val_masked_ids[val_masked_ids != 0]
            #       val_batch_score = compute_score_with_logits(val_logits, val_masked_ids)
            #       val_batch_acc = torch.sum(val_batch_score.float()) / torch.sum(val_inputs['masked_pos'])
            #   else:
            #       val_loss = scst_train_iter(args, val_dataset, model, scst_criterion, val_img_keys, val_batch, tokenizer)
            #       val_batch_acc = scst_criterion.get_score()
            #   if args.gradient_accumulation_steps > 1:
            #       val_loss = val_loss / args.gradient_accumulation_steps
            #   val_global_loss += val_loss.item()
            #   val_global_acc += val_batch_acc.item()
            #   if (val_step + 1) % args.gradient_accumulation_steps == 0:
            #       val_global_step += 1
            # add_val_log['loss'] = val_global_loss/val_global_step
            # add_val_log['acc'] = val_global_acc/val_global_step
            # log_val.add_val(**add_val_log)  
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            #|        END Get (epoch, loss, score) for valid dataset        |
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            ...
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            #|              Get metric-values for valid dataset             |
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            add_val_metrics_log.update(res)
            log_val_metrics.add_val(**add_val_metrics_log)
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
            #|            END Get metric-values for valid dataset           |
            #|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
...
def main():
    ...
    parser.add_argument('--current_loss', type=float, default=0.0, help="current loss")
    parser.add_argument('--current_acc', type=float, default=0.0, help="current acc")
    ...
    # global logger
    global log_train, log_val_metrics#, log_val
    ...
    if args.do_train:
        ...
        log_train = my_logger.Logger(args.output_dir, 'log_train.txt', ['epoch', 'loss', 'acc'], log_type='Train')
        # log_val = my_logger.Logger(args.output_dir, 'log_val.txt', ['epoch', 'loss', 'acc'], log_type='Valid')
        log_val_metrics = my_logger.Logger(args.output_dir, 'log_val_metrics.txt', ['epoch', 'Bleu_1', 'Bleu_2', 'Bleu_3', 'Bleu_4', 'METEOR', 'ROUGE_L', 'CIDEr'], log_type='Valid')
