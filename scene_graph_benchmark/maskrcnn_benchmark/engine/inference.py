...
def compute_on_dataset(cfg, model, data_loader, device, bbox_aug, labelmap_file='', output_folder=None, timer=None):
    model.eval()
    results_dict = {}
    cpu_device = torch.device("cpu")
    ...
    for idx, batch in enumerate(tqdm(data_loader)):
        ...
        if (cfg.LIMIT_DATA and idx != 0 and idx % cfg.LIMIT_DATA == 0) or idx==len(data_loader)-1:
          pred = _accumulate_predictions_from_multiple_gpus(results_dict, cfg.TEST.GATHER_ON_CPU)
          ...
          if cfg.LIMIT_DATA==None or len(data_loader)<=cfg.LIMIT_DATA:
            ...
            output_tsv_name = 'predictions_{}.tsv'.format(np.ceil(idx/cfg.LIMIT_DATA).astype(int))
          convert_predictions_to_tsv(
              pred, dataset, output_folder,
...
def inference(
    ...
    if output_folder and os.path.isfile(os.path.join(output_folder, output_pth_name)):
        ...
        if eval_attributes:
            ...
            #                                  inference_timer)
            compute_on_dataset(cfg, model, data_loader, device, bbox_aug, labelmap_file=labelmap_file,
                                             timer=inference_timer, output_folder=output_folder)
